...
// @ts-ignore
export const part1 = ins => ins.filter((_v, i, arr) => i >= 1 ? parseInt(arr[i-1]) < parseInt(arr[i]) : false).length.toString() 

// @ts-ignore
export const part2 = ins => ins.filter((_v, i, arr) => i >= 3 ? arr.slice(i-3,i).reduce((acc, s) => acc + parseInt(s), 0) < arr.slice(i-2,i+1).reduce((acc, s) => acc + parseInt(s), 0) : false).length.toString()
